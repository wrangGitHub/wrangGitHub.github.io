j12.iUtil = {
    getPosition: function(e) {
        ...
        var el = e;
        while (el) {
            x += el.offsetLeft + (el.currentStyle && !j12.browser.opera ? parseInt(el.currentStyle.borderLeftWidth) || 0 : 0);
            y += el.offsetTop + (el.currentStyle && !j12.browser.opera ? parseInt(el.currentStyle.borderTopWidth) || 0 : 0);
            el = el.offsetParent
        }
        el = e;
    ...
    getSize: function(e) {
        var w = j12.css(e, 'width');
        var h = j12.css(e, 'height');
        var wb = 0;
        var hb = 0;
        var es = e.style;
        if (j12(e).css('display') != 'none') {
    ...
    traverseDOM: function(nodeEl, func) {
        ...
        while (nodeEl) {
            j12.iUtil.traverseDOM(nodeEl, func);
            nodeEl = nodeEl.nextSibling
        }
    },
    purgeEvents: function(nodeEl) {
        j12.iUtil.traverseDOM(nodeEl, function(el) {
            for (var attr in el) {
...
j12.fx.destroyWrapper = function(e, old) {
    var c = e.firstChild;
    var cs = c.style;
...
j12.fx.buildWrapper = function(e) {
    if (!j12.fxCheckTag(e)) return false;
    var t = j12(e);
    var es = e.style;
    var restoreStyle = false;
    var oldStyle = {};
    ...
    oldStyle.sizes = j12.iUtil.getSize(e);
    oldStyle.margins = j12.iUtil.getMargins(e);
...
j12.fx.parseColor = function(color, notColor) {
    if (j12.fx.namedColors[color]) return {
        r: j12.fx.namedColors[color][0],
        g: j12.fx.namedColors[color][1],
        b: j12.fx.namedColors[color][2]
    };
...
j12.fx.cssSides = ['Top', 'Right', 'Bottom', 'Left'];
j12.fx.cssSidesEnd = {
...
j12.fn.extend({
    animate: function(prop, speed, easing, callback) {
        return this.queue(function() {
            var opt = j12.speed(speed, easing, callback);
            var e = new j12.fxe(this, opt, prop)
        })
    },
    pause: function(speed, callback) {
        return this.queue(function() {
            var opt = j12.speed(speed, callback);
            var e = new j12.pause(this, opt)
        })
    },
    stop: function(step) {
        return this.each(function() {
            if (this.animationHandler) j12.stopAnim(this, step)
        })
    },
...
j12.extend({
    pause: function(elem, options) {
        var z = this,
            values;
        z.step = function() {
            if (j12.isFunction(options.complete)) options.complete.apply(elem)
        };
    ...
    fxe: function(elem, options, prop) {
        var z = this,
            values;
        var y = elem.style;
        var oldOverflow = j12.css(elem, "overflow");
        var oldDisplay = j12.css(elem, "display");
        var props = {};
        z.startTime = (new Date()).getTime();
        options.easing = options.easing && j12.easing[options.easing] ? options.easing : 'linear';
        z.getValues = function(tp, vp) {
            if (j12.fx.cssProps[tp]) {
                if (vp == 'show' || vp == 'hide' || vp == 'toggle') {
                    if (!elem.orig) elem.orig = {};
                    var r = parseFloat(j12.curCSS(elem, tp));
                    elem.orig[tp] = r && r > -10000 ? r : (parseFloat(j12.css(elem, tp)) || 0);
                    ...
                    else j12.attr(y, "opacity", props[tp][0])
                } else {
                    props[tp] = [parseFloat(j12.curCSS(elem, tp)), parseFloat(vp) || 0]
                }
            } else if (j12.fx.colorCssProps[tp]) props[tp] = [j12.fx.parseColor(j12.curCSS(elem, tp)), j12.fx.parseColor(vp)];
                ...
                switch (tp) {
                    ...
                    case 'borderColor':
                        ...
                        m[2] = m[2] || m[0];
                        m[1] = m[1] || m[0];
                        for (var i = 0; i < j12.fx.cssSides.length; i++) {
                            var nmp = j12.fx.cssSidesEnd[tp][0] + j12.fx.cssSides[i] + j12.fx.cssSidesEnd[tp][1];
                            props[nmp] = tp == 'borderColor' ? [j12.fx.parseColor(j12.curCSS(elem, nmp)), j12.fx.parseColor(m[i])] : [parseFloat(j12.curCSS(elem, nmp)), parseFloat(m[i])]
                        }
                        break;
                    case 'border':
                        for (var i = 0; i < m.length; i++) {
                            ...
                            if (sideEnd) {
                                for (var j = 0; j < j12.fx.cssSides.length; j++) {
                                    nmp = 'border' + j12.fx.cssSides[j] + sideEnd;
                                    props[nmp] = sideEnd == 'Color' ? [j12.fx.parseColor(j12.curCSS(elem, nmp)), j12.fx.parseColor(m[i])] : [parseFloat(j12.curCSS(elem, nmp)), floatVal]
                                }
                            } else {
                                y['borderStyle'] = m[i]
                            }
        ...
        for (p in prop) {
            if (p == 'style') {
                var newStyles = j12.parseStyle(prop[p]);
                for (np in newStyles) {
                        ...
                        if (cssRules) {
                            for (var j = 0; j < cssRules.length; j++) {
                                if (cssRules[j].selectorText == '.' + prop[p]) {
                                    ...
                                    var newStyles = j12.parseStyle(styles.replace(rule, '').replace(/}/g, ''));
                                    for (np in newStyles) {
                                        this.getValues(np, newStyles[np])
                                    }
                                }
        ...
        z.step = function() {
            ...
            if (t > options.duration + z.startTime) {
                ...
                z.timer = null;
                for (p in props) {
                    if (p == "opacity") j12.attr(y, "opacity", props[p][1]);
                ...
                if (options.hide || options.show)
                    for (var p in elem.orig)
                        if (p == "opacity") j12.attr(y, p, elem.orig[p]);
                        else y[p] = "";
                ...
                for (p in props) {
                    if (typeof props[p][1] == 'object') {
                        y[p] = 'rgb(' + parseInt(j12.easing[options.easing](pr, n, props[p][0].r, (props[p][1].r - props[p][0].r), options.duration)) + ',' + parseInt(j12.easing[options.easing](pr, n, props[p][0].g, (props[p][1].g - props[p][0].g), options.duration)) + ',' + parseInt(j12.easing[options.easing](pr, n, props[p][0].b, (props[p][1].b - props[p][0].b), options.duration)) + ')'
                    } else {
                        var pValue = j12.easing[options.easing](pr, n, props[p][0], (props[p][1] - props[p][0]), options.duration);
                        if (p == "opacity") j12.attr(y, "opacity", pValue);
...
j12.parseStyle = function(styles) {
    var newStyles = {};
    if (typeof styles == 'string') {
        ...
        for (var i = 0; i < styles.length; i++) {
            rule = styles[i].split(':');
            if (rule.length == 2) {
                newStyles[j12.trim(rule[0].replace(/\-(\w)/g, function(m, c) {
...
j12.iFisheye = {
    build: function(options) {
        return this.each(function() {
            var el = this;
            el.fisheyeCfg = {
                items: j12(options.items, this),
                container: j12(options.container, this),
                pos: j12.iUtil.getPosition(this),
            ...
            j12.iFisheye.positionContainer(el, 0);
            j12(window).bind('resize', function() {
                el.fisheyeCfg.pos = j12.iUtil.getPosition(el);
                j12.iFisheye.positionContainer(el, 0);
                j12.iFisheye.positionItems(el)
            });
            j12.iFisheye.positionItems(el);
            el.fisheyeCfg.items.bind('mouseover', function() {
                j12(el.fisheyeCfg.itemsText, this).get(0).style.display = 'block'
            }).bind('mouseout', function() {
                j12(el.fisheyeCfg.itemsText, this).get(0).style.display = 'none'
            });
            j12(document).bind('mousemove', function(e) {
                var pointer = j12.iUtil.getPointer(e);
                var toAdd = 0;
                ...
                j12.iFisheye.positionContainer(el, toAdd)
            })
        })
    },
...
j12.fn.Fisheye = j12.iFisheye.build;
